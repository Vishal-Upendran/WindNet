...
if path_for_model=='NaiveMean':
    path =  'Models/NaiveMean/193/CV_'+str(cv)+'/Persist'+str(history)+str(delay)+'/'
    ...
    path = 'Models/Persistence/211/CV_'+str(cv)+'/Persist'+str(history)+str(delay)+'/'
    datapath = path+'/Testing/Persist_Predict_Test_Iter_no0.npy'
    Data = np.load(datapath,allow_pickle=True).tolist()
    Error = Data['stddev']
    Prediction = Data['data']
elif path_for_model.split('_')[0]=='SWSVM':
    path = 'Models/SVM_SW/193/CV_'+str(cv)+'/SVM_SW'+str(history)+str(delay)+'/'
    datapath = path + '/Testing/'+path_for_model.split('_')[-1]+'_Predict_Test_Iter_no0.npy'
    Data = np.load(datapath,allow_pickle=True).tolist()
    Error = Data['stddev']
    Prediction = Data['data']
elif path_for_model.split('_')[0]=='XGBoost':
    path = 'Models/XGBoost_SW/193/CV_'+str(cv)+'/XGBoost_SW'+str(history)+str(delay)+'/'
